...
Cada cartão possui ações para concluir ou reabrir, editar e excluir. O campo de busca procura em todos os compromissos do dia, e os filtros ajudam a visualizar apenas pendências ou itens já concluídos. Para transportar os dados entre navegadores, use **Exportar** e depois **Importar** com o arquivo JSON gerado.
...
O cronograma acadêmico detalhado está em [`CRONOGRAMA.md`](./CRONOGRAMA.md). Entre as evoluções planejadas estão visão semanal, categorias com cores, lembretes, testes automatizados de regras e sincronização opcional com uma API.
